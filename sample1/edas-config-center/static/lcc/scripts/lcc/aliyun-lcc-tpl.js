...
angular.module("scripts/lcc/views/resource/serviceList.html", []).run(["$templateCache", function($templateCache) {
  ...
  $templateCache.put("scripts/lcc/views/resource/serviceList.html",
    ...
    "            \n" +
    "      </div> --><div class=\"form-group col-sm-3\" style=\"text-align:center\"><button class=\"btn btn-primary\" ng-click=\"search()\" style=\"margin-left: 20px\" ng-disabled=\"!ip && !dataId\">搜索</button> <span ng-show=\"errorTips\" class=\"text-danger\">{{errorTips}}</span></div><div style=\"margin: 0;padding: 0 0 0 80px;color: #999;clear: both\">请至少填写IP地址或服务名其中一项进行搜索</div></form></div><ul class=\"nav nav-tabs\"><li ng-repeat=\"tab in tabs\" ng-class=\"{active:$state.includes('serviceList.' + tab.state)}\"><a aliyun-popover2=\"{{tab.tips}}\" ui-sref=\"serviceList.{{tab.state}}\">{{tab.name}}</a></li></ul><div ui-view=\"serviceView\"></div></div></div>");
}]);
